#ifndef INC_D3DatabasePermissionBase_H
#define INC_D3DatabasePermissionBase_H
...
// D3DatabasePermission representing instances of D3MDDB.D3DatabasePermission.
//
...
//       namespace D3
//       {
//         class D3DatabasePermission : public D3DatabasePermissionBase
//         {
...
namespace D3
{
	...
	//! Use these enums to access columns through D3DatabasePermission::Column() method
	enum D3DatabasePermission_Fields
	{
		D3DatabasePermission_RoleID,
		D3DatabasePermission_MetaDatabaseID,
		D3DatabasePermission_AccessRights
	};


	//! D3DatabasePermissionBase is a base class that \b MUST be subclassed through a class called \a D3DatabasePermission.
	/*! The purpose of this class to provide more natural access to related objects as well as this' columns.
			This class is only usefull if it is subclassed by a class called D3DatabasePermission
			...
			dictionary will D3 instantiate objects of type \a D3DatabasePermission representing rows from the table \a D3MDDB.D3DatabasePermission.
	*/
	class D3_API D3DatabasePermissionBase: public Entity
	{
		D3_CLASS_DECL(D3DatabasePermissionBase);
		...
		protected:
			D3DatabasePermissionBase() {}

		public:
			~D3DatabasePermissionBase() {}

			//! Create a new D3DatabasePermission
			static D3DatabasePermissionPtr      CreateD3DatabasePermission(DatabasePtr pDB)		{ return (D3DatabasePermissionPtr) pDB->GetMetaDatabase()->GetMetaEntity(D3MDDB_D3DatabasePermission)->CreateInstance(pDB); }
			...
			static D3DatabasePermissionPtr      Load(DatabasePtr pDB, long lRoleID, long lMetaDatabaseID, bool bRefresh = false, bool bLazyFetch = true);
			...
			*/
			//@{
			//! RoleID
			virtual long                        GetRoleID()                    { return Column(D3DatabasePermission_RoleID)->GetLong(); }
			//! MetaDatabaseID
			virtual long                        GetMetaDatabaseID()            { return Column(D3DatabasePermission_MetaDatabaseID)->GetLong(); }
			//! AccessRights
			virtual long                        GetAccessRights()              { return Column(D3DatabasePermission_AccessRights)->GetLong(); }
			//@}
			...
			*/
			//@{
			//! Set RoleID
			virtual bool												SetRoleID(long val)            { return Column(D3DatabasePermission_RoleID)->SetValue(val); }
			//! Set MetaDatabaseID
			virtual bool												SetMetaDatabaseID(long val)    { return Column(D3DatabasePermission_MetaDatabaseID)->SetValue(val); }
			//! Set AccessRights
			virtual bool												SetAccessRights(long val)      { return Column(D3DatabasePermission_AccessRights)->SetValue(val); }
			//@}

			//! A column accessor provided mainly for backwards compatibility
			virtual ColumnPtr										Column(D3DatabasePermission_Fields eCol){ return Entity::GetColumn((unsigned int) eCol); }
	};
...
#endif /* INC_D3DatabasePermissionBase_H */
